...
`bacon` uses Convex as the default database provider. If you are using the Prisma adapter stub, this guide will help you switch the database provider from Neon to EdgeDB. For authentication, bacon uses Better Auth (not Clerk).
...
Here's how to switch from Neon to [EdgeDB](https://edgedb.com) for your `bacon` project.
