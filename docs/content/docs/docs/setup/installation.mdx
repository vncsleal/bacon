...
Run the `bacon` init command:

```package-install
npx bacon@latest init
```
...
For more details on the default Prisma configuration (using Neon), refer to the [Database Configuration Guide](https://www.bacon.com/packages/database#default-configuration).
