---
title: Payments
description: How bacon handles payments and billing.
---

bacon uses [Stripe](https://stripe.com/) by default for payments and billing. Implementing Stripe in your project is straightforward.
...
bacon has a pre-built webhook handler for payment events. Read more about [inbound payment webhooks](/packages/webhooks/inbound#payment-events).
...
Stripe Radar supports more [advanced anti-fraud features](https://docs.stripe.com/disputes/prevention/advanced-fraud-detection) if you embed the Stripe JS script in every page load. This is not enabled by default in bacon, but you can add it as follows:
